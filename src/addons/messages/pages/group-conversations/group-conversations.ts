...
export class AddonMessagesGroupConversationsPage implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.newMessagesObserver = CoreEvents.on(
            ...
            (data) => {
                ...
                if (conversation.lastmessage !== data.message || conversation.lastmessagedate !== data.timecreated / 1000) {
                    ...
                    const option = this.getConversarionGroupByName(optionName);
        ...
        this.memberInfoObserver = CoreEvents.on(
            ...
            (data) => {
                ...
                const expandedOption = this.getExpandedOption();
                if (expandedOption?.optionName === 'group' || !expandedOption?.conversations.length) {
                    return;
                }
    ...
    protected async fetchDataForExpandedOption(): Promise<void> {
        if (!this.firstExpand) {
            ...
            if (!expandOption) {
                expandOption = this.getConversarionGroupByName(AddonMessagesGroupConversationOptionNames.INDIVIDUAL);
            }
    ...
    protected setCounts(
        ...
        this.getConversarionGroupByName(AddonMessagesGroupConversationOptionNames.FAVOURITES)[valueToSet] = counts.favourites;
        this.getConversarionGroupByName(AddonMessagesGroupConversationOptionNames.INDIVIDUAL)[valueToSet] =
            counts.individual + counts.self; // Self is only returned if it's not favourite.
            this.getConversarionGroupByName(AddonMessagesGroupConversationOptionNames.GROUP)[valueToSet] = counts.group;
    }
     ...
     */
    protected getConversarionGroupByName(name: AddonMessagesGroupConversationOptionNames): AddonMessagesGroupConversationOption {
    ...
    protected findConversation(
        ...
        if (!conversations) {
            ...
            conversations = this.getConversarionGroupByName(AddonMessagesGroupConversationOptionNames.FAVOURITES).conversations
                .concat(this.getConversarionGroupByName(AddonMessagesGroupConversationOptionNames.INDIVIDUAL).conversations);
        }
    ...
    protected getExpandedOption(): AddonMessagesGroupConversationOption | undefined {
        if (this.accordionGroup.value) {
            return this.getConversarionGroupByName(this.accordionGroup.value as AddonMessagesGroupConversationOptionNames);
        }
    }
